...
from buildbot.status.results import SUCCESS, WARNINGS, FAILURE, SKIPPED,NOBUILD
...
#wdong
def getAndCheckProperties_marvell(req):
    """
    Fetch custom build properties from the HTTP request of a "Force build" or
    "Resubmit build" HTML form.
    Check the names for valid strings, and return None if a problem is found.
...
def path_to_query(request):
    return path_to_root(request) + "query"
    
def path_to_trigger(request):
    return path_to_root(request) + "trigger"
...
class DownLoadResource(resource.Resource, ContextMixin):
    ...
    def render(self, request):
        ...
        request.setHeader("Content-disposition", self.contentDisposition)
        data = self.getData() 
...
class HtmlResource(resource.Resource, ContextMixin):
    ...
    #add by zlinghu begin:20111214------if user is not in cookie ,redirect to login page
    def contentLogin(self, req, context):
...
#add by zlinghu end  
    ...
    def render(self, request):
        ...
        if ((request.getCookie('username') and request.getCookie('dispname')) or request.getUri() == "/authfail"):
            d = defer.maybeDeferred(lambda : self.content(request, ctx))
        else:
            #if(request.getUri().find("authfail")):
            #	d = defer.maybeDeferred(lambda : self.content(request, ctx))
            #else:
            d = defer.maybeDeferred(lambda : self.contentLogin(request, ctx))
        def handle(data):
            if isinstance(data, unicode):
                log.msg("msgis" + data)
                data = data.encode("utf-8")
...
class BuildLineMixin:
    ...
    def get_line_values(self, req, build, include_builder=True):
        ...
        text = build.getText()
        text1 = build.getText1()
        reason1 = build.getFailureReason()
        rev = str(build.getProperty("got_revision", "unknown"))
        branch = str(build.getProperty("branch" , "none"))
        css_class = css_classes.get(results, "")
        repo = build.getSourceStamp().repository
        source = build.getSourceStamp()
        ...
        values = {'class': css_class,
                  ...
                  'text': text,
                  'text1':"".join(text1),
                  'text2':"".join(text1).split('/')[-1],
                  'reason1':"".join(reason1),
                  #wdong
                  #'branch':branch,
                  'include_builder': include_builder,
                  #add by zlinghu begin:20111125
                  'branch':source.branch,
                  'ssid': source.ssid,
                  'reason': build.getReason(),
                  'edittime': build.getEditTime(),
                  'editperson': build.getEditPerson()
                  #add by zlinghu end
                  }
        return values
