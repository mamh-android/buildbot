...
from buildbot.status.results import SUCCESS, WARNINGS, FAILURE, SKIPPED, \
      EXCEPTION, RETRY, worst_status, NOBUILD
...
class LoggingBuildStep(BuildStep):
    ...
    def maybeGetText2(self, cmd, results):
        if results == SUCCESS:
            ...
            #pass
           if self.name == "shell":
               first=cmd.logs['stdio'].getText().find("<result-dir>")
               last=cmd.logs['stdio'].getText().find("</result-dir>")
               if first != -1:
                   return [cmd.logs['stdio'].getText()[first:last].split("<result-dir>")[1]]
           else:
               return ["success"]
        elif results == WARNINGS:
