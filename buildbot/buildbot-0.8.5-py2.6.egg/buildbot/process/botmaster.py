...
class BotMaster(service.MultiService):
    ...
    def maybeStartBuildsForBuilder(self, buildername):
        ...
        """
        #wdong
        log.msg("maybeStartBuildsForBuilder is",buildername)
        #end
    ...
    def maybeStartBuildsForSlave(self, slave_name):
        ...
        #wdong
        log.msg("maybeStartBuildsForSlave is",builders)
        #end
...
class BuildRequestDistributor(service.Service):
    ...
    def maybeStartBuildsOn(self, new_builders):
        ...
        #wdong
        log.msg("existing_pending | new_builders is", list(existing_pending | new_builders))
        #end
    ...
    def _defaultSorter(self, master, builders):
        ...
        def xform(bldr):
            ...
            log.msg("getOldestRequestTime is", d)
            return d
        ...
        def nonecmp(a,b):
            ...
            if(a[0]['priority'] > b[0]['priority']):
                return -1
            elif a[0]['priority'] ==b[0]['priority']: 
                if (a[0]['submitted_at']<= b[0]['submitted_at']):
                    return -1
            #wdong
            return 1
        xformed.sort(cmp=nonecmp)
        #xformed.sort(key=lambda x:x[0]['submitted_at'])
        #xformed.sort(key=lambda x:x[0]['priority'])
        log.msg("xformed sorted is", xformed)

        # and reverse the transform
        #yield [ xf['buildername'] for xf in xformed ]
    ...
    def _sortBuilders(self, buildernames):
        ...
        try:
            ...
            log.msg("sort builders is", builders)
        except:
