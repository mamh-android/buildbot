...
class OneBuildSlaveResource(HtmlResource, BuildLineMixin):
    ...
    def getCpuInfo(self):
        ...
        cache = []
        for line in lines:
            if line.split(':')[0].rstrip() == 'processor':
                cpu.append(line.split(':')[1].strip())
            if line.split(':')[0].rstrip() == 'vendor_id':
                vendor.append(line.split(':')[1].strip())
            if line.split(':')[0].rstrip() == 'model name':
                name.append(line.split(':')[1].strip())
            if line.split(':')[0].rstrip() == 'cpu MHz':
                cpuMHz.append(line.split(':')[1].strip())
            if line.split(':')[0].rstrip() == 'cache size':
                cache.append(line.split(':')[1].strip())
        cpun = len(cpu)
        Cpu = []
        i = 0
        for cpui in cpu:
            Cpu.append({'vendor':vendor[i],'name':name[i],'cpuMHz':cpuMHz[i],'cache':cache[i]})
            i = i + 1
        return Cpu
    ...
    def getMemInfo(self):
        ...
        f.close()
        for line in lines:
            if line.split(':')[0].rstrip() == 'MemTotal':
                memtotal=(line.split(':')[1].strip())
            if line.split(':')[0].rstrip() == 'MemFree':
                memfree=(line.split(':')[1].strip())
            if line.split(':')[0].rstrip() == 'Cached':
                cache=(line.split(':')[1].strip())
        Mem = [{'total':memtotal,'free':memfree,'cache':cache}]
        return Mem
...
class BuildSlavesResource(HtmlResource):
    ...
    def getChild(self, path, req):
        if path == "restart":
            return RestartActionResource(req)
        try:
...
class RestartActionResource(ActionResource):
    ...
    def performAction(self, req):
        self.slaveInfo = req.args.get("slaveInfo", [""])[0]
        self.slaveIp = self.slaveInfo[self.slaveInfo.find("IP:") + 3 : ].strip()
        self.user = "buildfarm"
        self.passwd = "shmarvell"
        client = paramiko.SSHClient()
        client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        try:
            client.connect(self.slaveIp, 22, username=self.user, password=self.passwd, timeout=10)
        except:  
            log.msg("restart error!")
        else:
            log.msg("begin to restart") 
            stdin, stdout, stderr = client.exec_command('sudo reboot')
            print stdout.readlines()
            print stderr.readlines()
            req.addCookie('rebootip', self.slaveIp, path = '/', expires = time.strftime("%a     %b %d %H:%M:%S %Y", time.gmtime(time.time()+0.5*3600)))
        client.close()
        yield "../buildslaves"
