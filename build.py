...
from buildbot.status.web.base import HtmlResource, \
     ...
     getAndCheckProperties, ActionResource, path_to_authfail, path_to_query
...
class ForceBuildActionResource(ActionResource):
    ...
    def performAction(self, req):
        ...
        yield wfd
        res = wfd.getResult()
        #add by zlinghu to judge wether the branch is in running or pending
        branch = self.build_status.getSourceStamp().branch
        num = self.build_status.getNumber()
        ...
        for buildername in buildernames:
            ...
            #for x in b.getCurrentBuilds():
                #log.msg("builderis")
            cxt['current'] = [self.getBuilder(x, req) for x in b.getCurrentBuilds()]
        ...
        if not res:
            ...
            name = req.getCookie("dispname")
            comments = req.args.get("comments", ["no reason specified"])[0]
            reason = ("%s rebuild, reason: %s \n" % (name, comments))
