...
def return_json_str_list(project, branch):
    cmd = "ssh -p 29418 %s@%s gerrit query --current-patch-set --format=JSON NOT age:5day project:^.*%s branch:%s is:open NOT Verified+1 NOT CodeReview-1 NOT CodeReview-2" % (m_user, m_remote_server, project, branch)
