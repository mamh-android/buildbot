...
def return_via_patch_approvals(in_array, category, value, change_open):
    ...
    for rows in in_array:
        a = []
        args = "ssh -p 29418 " + m_user + "@" + m_remote_server + " gerrit gsql -c \"select\ *\ from\ patch_set_approvals\ WHERE\ VALUE=\\\'" + value +  "\\\'\ AND\ CHANGE_OPEN=\\\'" + change_open + "\\\'\ AND\ CATEGORY_ID=\\\'" + category + "\\\'\ AND\ CHANGE_ID=\\\'" + rows[14].strip() + "\\\'\""
        f = open('tmptxt', 'w')
        f.write(os.popen(args).read())
        f.close()
        in_txt = csv.reader(open("tmptxt", "rb"), delimiter = '|')
...
def main(argv):
    ...
    review_category = ""
    try:
        opts, args = getopt.getopt(argv, "o:b:p:h",["review="])
